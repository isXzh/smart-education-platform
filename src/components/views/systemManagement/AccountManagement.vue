<template>
  <div class="p-6 common-container">
            ...
            <div>
              ...
              <div class="flex items-center gap-3 mt-1">
                <span class="text-xs text-gray-500">
                  总计
                  <span class="text-blue-600 font-medium">{{ accountStats.total }}</span>
                </span>
                <span class="text-xs text-gray-500">
                  启用
                  <span class="text-green-600 font-medium">{{ accountStats.enabled }}</span>
                </span>
                <span class="text-xs text-gray-500">
                  禁用
                  <span class="text-red-600 font-medium">{{ accountStats.disabled }}</span>
                </span>
              </div>
            </div>
          </div>
      ...
      <div class="p-0">
        ...
        >
          ...
          <el-table-column prop="roleName" label="角色" min-width="140">
            <template slot-scope="{ row }">
              <el-tag :type="getRoleType(row.roleCode)" size="small" effect="light" class="!rounded-full">
          ...
          <el-table-column prop="lastLoginAt" label="最后登录时间" min-width="160">
            <template slot-scope="{ row }">
              <div>{{ row.lastLoginAt ? row.lastLoginAt.split('T').join(' ') : '-' }}</div>
            </template>
          </el-table-column>
          <el-table-column label="操作" fixed="right" min-width="180">
            <template slot-scope="{ row }">
              <div class="flex items-center gap-2">
                <el-button type="text" size="small" class="!text-blue-600" @click="handleEdit(row)">
                  <i class="el-icon-edit mr-1"></i>编辑
                </el-button>
                <el-button type="text" size="small" class="!text-amber-600" @click="handleResetPassword(row)">
                  <i class="el-icon-key mr-1"></i>重置密码
                </el-button>
                <el-button type="text" size="small" class="!text-gray-400" @click="handleDisable(row)">
                  <i :class="row.status === '0' ? 'el-icon-circle-close' : 'el-icon-circle-check'" class="mr-1"></i>
                  {{ row.status === '0' ? '禁用' : '启用' }}
                </el-button>
              </div>
            </template>
      ...
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1">
          ...
          <ul class="text-xs text-blue-700 space-y-1">
            ...
            <li>• 定期更新密码，建议密码长度不少于8位，包含字母、数字和特殊字符</li>
    ...
    <!-- 重置密码弹窗 -->
    <ResetPasswordDialog :visible.sync="resetPasswordVisible" :data="currentRow" @success="handleResetSuccess" />
  </div>
</template>
