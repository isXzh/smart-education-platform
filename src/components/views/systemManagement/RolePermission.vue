<template>
  <div class="p-6 common-container">
        ...
        <div class="w-80 border-r border-gray-100">
          <div class="p-5 border-b border-gray-100">
            <div class="flex items-center justify-between mb-4">
              ...
              <el-button type="primary" size="small" circle @click="handleAddRole">
                <i class="el-icon-plus"></i>
              </el-button>
            </div>
            <div class="text-xs text-blue-600 bg-blue-50 rounded-full px-3 py-1 inline-block">
              共 {{ roles.length }} 个角色
            </div>
          </div>
          ...
          <div class="overflow-y-auto" style="max-height: 600px">
            <div
              ...
              :class="{
                'bg-blue-50 border-l-4 border-l-blue-500': selectedRole && selectedRole.id === role.id,
              }"
              @click="selectRole(role)"
            >
              <div class="flex items-center justify-between">
                <div>
                  <div class="font-medium text-gray-800">{{ role.roleName }}</div>
                  <div class="text-xs text-gray-500 mt-1">
                    {{ role.remark }}
                  </div>
                  <div class="text-xs text-gray-400 mt-1">创建时间：{{ role.createdAt }}</div>
                </div>
                <div class="flex gap-1" v-if="selectedRole && selectedRole.id === role.id">
                  <el-button type="text" size="mini" @click.stop="handleEditRole(role)">
                    <i class="el-icon-edit text-blue-500"></i>
                  </el-button>
                  <el-button type="text" size="mini" @click.stop="handleDeleteRole(role)">
          ...
          <div v-if="selectedRole" class="h-full flex flex-col">
            ...
            <div class="flex-1 border border-gray-200 rounded-lg overflow-hidden">
              <div class="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-center">
                <div class="flex-1 text-sm font-medium text-gray-700">模块/功能</div>
              ...
              <div class="overflow-y-auto" style="max-height: 580px">
                <div v-for="module in permissionTree" :key="module.id" class="border-b border-gray-100">
                  <!-- 模块行 -->
                  <div class="flex items-center px-4 py-3 hover:bg-gray-50">
                    <div class="flex-1 flex items-center gap-2">
                      <i class="el-icon-arrow-down text-gray-400"></i>
                      <!-- <i class="el-icon-arrow-right text-gray-400" v-else></i> -->
                      <el-checkbox v-model="module.checked" @change="handleModuleCheck(module)">
                        <span class="font-medium text-gray-800">{{ module.label || module.name }}</span>
                      </el-checkbox>
                    </div>
                    <div class="w-20 text-center">
                      <el-checkbox v-model="module.permissions.view" @change="updateModuleCheck(module)"></el-checkbox>
                    </div>
                    <div class="w-20 text-center">
                      <el-checkbox v-model="module.permissions.add" @change="updateModuleCheck(module)"></el-checkbox>
                    </div>
                    <div class="w-20 text-center">
                      <el-checkbox v-model="module.permissions.edit" @change="updateModuleCheck(module)"></el-checkbox>
                    </div>
                    <div class="w-20 text-center">
                      <el-checkbox
                  ...
                  <!-- 子功能行 -->
                  <div v-if="module.expanded && module.children" class="bg-gray-50/50">
                    <div
                    ...
                    >
                      <div class="flex-1 flex items-center gap-2">
                        ...
                        <el-checkbox v-model="child.checked" @change="handleChildCheck(child, module)">
                          <span class="text-sm text-gray-700">{{ child.label || child.name }}</span>
                        </el-checkbox>
                      </div>
                      <div class="w-20 text-center">
                        <el-checkbox
    ...
    <!-- 新增角色弹窗 -->
    <AddRoleDialog :visible.sync="addRoleVisible" @success="handleRoleSuccess" />

    <!-- 编辑角色弹窗 -->
    <EditRoleDialog :visible.sync="editRoleVisible" :data="currentRoleData" @success="handleRoleSuccess" />

    <!-- 加载权限模板弹窗 -->
    <LoadTemplateDialog :visible.sync="loadTemplateVisible" @select="handleTemplateSelect" />
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    async created() {
      await this.treeForPermission();
      await this.fetchRoles();
      if (this.roles.length > 0) {
        this.selectedRole = this.roles[0];
        await this.getById(this.roles[0].id);
      }
    },
    methods: {
      async fetchRoles() {
      ...
      async getById(id) {
        try {
          const res = await role.getById(id);
          if (res.code === 200) {
            const roleData = res.data;
            this.roleIds = roleData.menuIds || [];
            
            if (this.permissionTree && this.permissionTree.length > 0) {
              this.checkPermissions(this.permissionTree, this.roleIds);
            }
          }
        } catch (error) {
      ...
      async treeForPermission() {
        try {
          const res = await menu.treeForPermission();
          if (res.code === 200) {
            const arr = res.data;
            arr.forEach(module => {
              ...
              if (module.children && module.children.length > 0) {
                module.children.forEach(child => {
                  child.checked = false;
                  child.permissions = { view: false, add: false, edit: false, delete: false };
                  
                  if (child.children && child.children.length > 0) {
                    child.children.forEach(permission => {
                      const permissionType = this.getPermissionType(permission.label);
                      if (permissionType) {
                        child.permissions[permissionType] = false;
                        child[permissionType + 'Id'] = permission.id;
                      }
                    });
            ...
            if (this.selectedRole) {
              await this.getById(this.selectedRole.id);
            }
          }
        } catch (error) {
      ...
      checkPermissions(tree, menuIds) {
        tree.forEach(module => {
          module.checked = false;
          module.permissions = { view: false, add: false, edit: false, delete: false };
          
          if (menuIds.includes(module.id)) {
            module.checked = true;
          ...
          if (module.children && module.children.length > 0) {
            module.children.forEach(child => {
              ...
              if (menuIds.includes(child.id)) {
                child.checked = true;
              }
              
              Object.keys(child.permissions).forEach(key => {
                const permissionId = child[key + 'Id'];
                if (permissionId && menuIds.includes(permissionId)) {
                  child.permissions[key] = true;
                }
              });
              
              const allPermissionsChecked = Object.values(child.permissions).every(v => v);
            ...
            const allChildrenChecked = module.children.every(child => child.checked);
      ...
      updateModuleCheck(module) {
        const allChecked =
          module.permissions.view && module.permissions.add && module.permissions.edit && module.permissions.delete;
      ...
      updateChildCheck(child, module) {
        const allChecked =
          child.permissions.view && child.permissions.add && child.permissions.edit && child.permissions.delete;
      ...
      updateParentCheck(module) {
        ...
        const allChildrenChecked = module.children.every(
          child => child.permissions.view && child.permissions.add && child.permissions.edit && child.permissions.delete
        );
